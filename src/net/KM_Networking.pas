...
uses
  ...
  KM_CommonClasses, KM_CommonTypes, KM_NetworkClasses, KM_NetworkTypes,
  KM_Defaults, KM_Saves, KM_GameOptions, KM_ResLocales, KM_NetFileTransfer,
...
type
  ...
  TNetGameState  = (
    lgs_None, lgs_Connecting, lgs_Query, lgs_Lobby, lgs_Loading, lgs_Game,
    lgs_Reconnecting
  );
  TNetGameKind   = (ngk_None, ngk_Map, ngk_Save);
  TChatSound     = (
    csNone, csJoin, csLeave, csSystem, csGameStart, csSaveGame, csChat,
    csChatWhisper, csChatTeam
  );
...
const
  NetMPGameState:    array [TNetGameState] of TMPGameState = (
    mgsNone, mgsNone, mgsNone, mgsLobby, mgsLoading, mgsGame, mgsGame
  );
  NetAllowedPackets: array [TNetGameState] of set of TKMessageKind = (
    ...
    //lgs_Connecting
    [
      mk_RefuseToJoin, mk_IndexOnServer, mk_GameVersion, mk_WelcomeMessage,
      mk_Ping, mk_ConnectedToRoom, mk_PingInfo, mk_Kicked, mk_ServerName,
      mk_ReqPassword
    ],
    //lgs_Query
    [
      mk_AllowToJoin, mk_RefuseToJoin, mk_AuthChallenge, mk_Ping, mk_PingInfo,
      mk_Kicked
    ],
    //lgs_Lobby
    [
      mk_AskForAuth, mk_AskToJoin, mk_ClientLost, mk_ReassignHost, mk_Disconnect,
      mk_Ping, mk_PingInfo, mk_PlayersList, mk_StartingLocQuery, mk_SetTeam,
      mk_FlagColorQuery, mk_ResetMap, mk_MapSelect, mk_SaveSelect,
      mk_ReadyToStart, mk_Start, mk_TextChat, mk_Kicked, mk_LangCode,
      mk_GameOptions, mk_ServerName, mk_FileRequest, mk_FileChunk, mk_FileEnd,
      mk_FileAck, mk_TextTranslated, mk_HasMapOrSave
    ],
    //lgs_Loading
    [
      mk_AskForAuth, mk_ClientLost, mk_ReassignHost, mk_Disconnect, mk_Ping,
      mk_PingInfo, mk_PlayersList, mk_ReadyToPlay, mk_Play, mk_TextChat,
      mk_Kicked, mk_TextTranslated, mk_Vote
    ],
    //lgs_Game
    [
      mk_AskForAuth, mk_ClientLost, mk_ReassignHost, mk_Disconnect, mk_Ping,
      mk_PingInfo, mk_FPS, mk_PlayersList, mk_ReadyToReturnToLobby, mk_Commands,
      mk_TextChat, mk_ResyncFromTick, mk_AskToReconnect, mk_Kicked,
      mk_ClientReconnected, mk_TextTranslated, mk_Vote
    ],
    //lgs_Reconnecting
    [
      mk_IndexOnServer, mk_GameVersion, mk_WelcomeMessage, mk_Ping, mk_FPS,
      mk_ConnectedToRoom, mk_PingInfo, mk_PlayersList, mk_ReconnectionAccepted,
      mk_RefuseReconnect, mk_Kicked
    ]
  );
...
type
  ...
  private
    ...
    fServerName,                            // Name of the server we are currently in (shown in the lobby)
    fPassword:           AnsiString;
    fDescription:        UnicodeString;
    fEnteringPassword:   Boolean;
    fMyIndexOnServer,
    fMyIndex,                               // In NetPlayers list
    fHostIndex,                             // In NetPlayers list
  ...
  public
    ...
    procedure SelectLoc(aIndex, aPlayerIndex: Integer);
    procedure SelectTeam(aIndex, aPlayerIndex: Integer);
    procedure SelectColor(aIndex, aPlayerIndex: Integer);
...
implementation
uses
  KM_ResTexts, KM_Sound, KM_ResSound, KM_Log, KM_Utils, StrUtils, Math,
...
begin
  ...
  fNetServer                             := TKMDedicatedServer.Create(
    1, aKickTimeout, aPingInterval, aAnnounceInterval, aMasterServerAddress, '',
    '', False
  );
...
begin
  ...
  if (fNetGameState in [lgs_Loading, lgs_Game]) and
     (fNetPlayers.FurtherVotesNeededForMajority <= 0) then
...
begin
  ...
  for I := 1 to fNetPlayers.Count do
    for K := 1 to fSaveInfo.Info.PlayerCount do
      if fSaveInfo.Info.Enabled[K-1] and ((I = aPlayerID) or (aPlayerID = -1)) and//-1 means update all players
         fNetPlayers.LocAvailable(K) and fNetPlayers[I].IsHuman and
         not fNetPlayers[I].IsSpectator and
         (fNetPlayers[I].Nikname = fSaveInfo.Info.OwnerNikname[K-1]) then
      begin
...
procedure TKMNetworking.SelectLoc(aIndex, aPlayerIndex: Integer);
var
...
procedure TKMNetworking.SelectTeam(aIndex, aPlayerIndex: Integer);
begin
...
procedure TKMNetworking.SelectColor(aIndex, aPlayerIndex: Integer);
begin
...
var
  HumanUsableLocs,
  AIUsableLocs:    TKMHandIndexArray;
  ...
  if (fNetGameState = lgs_Lobby) and (fSelectGameKind = ngk_Save) then
    for i:=1 to NetPlayers.Count do
      ...
      begin
        ...
        if (NetPlayers[i].FlagColorID <> 0) and
           SaveInfo.Info.ColorUsed(NetPlayers[i].FlagColorID) then
...
begin
{
  ...
  begin
    ...
    if (Length(aText) >= s+1) and TryStrToInt(aText[s+1], PlayerID) and
       InRange(PlayerID, 1, fNetPlayers.Count) then
    begin
      if fNetPlayers[PlayerID].IsHuman and (PlayerID <> MyIndex) then
        KickPlayer(PlayerID)
      else
  ...
  end else if ConsoleCmd = '/help' then
...
const
  ChatSound: array[TChatSound] of TSoundFXNew = (
    sfxn_MPChatSystem, //csNone
...
begin
  Result := (fNetPlayers[aPlayerIndex].HandIndex <> -1) and
            (gHands[fNetPlayers[aPlayerIndex].HandIndex].AI.IsNotWinnerNotLoser) and // This means player is not defeated and not win
            not fNetPlayers[aPlayerIndex].IsSpectator
end;
...
begin
  ...
  begin
    ...
    if (fSelectGameKind = ngk_None) or
       ((fSelectGameKind = ngk_Map) and not MapInfo.IsValid) or
       ((fSelectGameKind = ngk_Save) and not SaveInfo.IsValid) then
    ...
    if fNetPlayers[OldHostIndex].Dropped and
       IsPlayerHandStillInGame(OldHostIndex) and
       (fNetPlayers[OldHostIndex].HandIndex <> -1) and
       Assigned(fOnJoinerDropped) then
...
begin
  ...
  if (aLoc = LOC_SPECTATE) and not fNetPlayers[aPlayer].IsSpectator then
    Result := Result and (
      (NetPlayers.SpectatorSlotsOpen = MAX_LOBBY_SPECTATORS) or //Means infinite spectators allowed
      (NetPlayers.SpectatorSlotsOpen - NetPlayers.GetSpectatorCount > 0)
    );
...
function TKMNetworking.GetNetAddressPrintDescr(aNetworkAddress: Integer): String;
  ...
  begin
    ...
    Result         := IfThen(NetPlayerIndex = -1, 'unknown', IntToStr(NetPlayerIndex));
    {
    if NetPlayerIndex = -1 then
      Result := 'unknown'
    else
...
begin
  ...
  case aKind of
    ...
    mk_Pong,
    mk_PingInfo,
    mk_FPS:      gLog.LogNetPacketPingFps(LogMessage);
    ...
    case Kind of
      ...
      mk_FileRequest:
        ...
        begin
          ...
          case fSelectGameKind of
            ngk_Map:  if ((tmpStringW <> MapInfo.FileName) and
                          tmpStringW <> MapInfo.FileName + '_' + IntToHex(MapInfo.CRC, 8))) or
                         not fFileSenderManager.StartNewSend(kttMap, MapInfo.FileName, MapInfo.MapFolder, aSenderIndex) then
                        PacketSend(aSenderIndex, mk_FileEnd); //Abort
            ngk_Save: if (tmpStringW <> SaveInfo.FileName) or
                         not fFileSenderManager.StartNewSend(kttSave, SaveInfo.FileName, mfDL, aSenderIndex) then
                        PacketSend(aSenderIndex, mk_FileEnd); //Abort
          end;
        end;
      ...
      mk_Ping:         PacketSend(aSenderIndex, mk_Pong); //Server will intercept this message
      ...
      mk_FlagColorQuery:
        ...
        begin
          ...
          if fNetPlayers.ColorAvailable(ColorID) and
             ((fSelectGameKind <> ngk_Save) or not SaveInfo.IsValid or
              not SaveInfo.Info.ColorUsed(ColorID)) then
          begin
      ...
      mk_Play: if fNetPlayerKind = lpk_Joiner then PlayGame;
      ...
      mk_Vote:
        begin
          ...
          if not fVoteReturnToLobbySucceeded and // Do not allow late mk_Vote after we received enought votes (if it comes while still in game and receiveing mk_readyToReturnToLobby)
             not fNetPlayers[PlayerIndex].VotedYes and //No need to vote more than once
             (fNetPlayers.HasOnlySpectators or not fNetPlayers[PlayerIndex].IsSpectator) then //spectators don't get to vote unless there's only spectators left
          begin
      ...
      mk_TextChat:
        begin
          ...
          case tmpChatMode of
            ...
            cmWhisper:
              begin
                ...
                tmpStringW := ' [$00B9FF](' + Format(gResTexts[TX_CHAT_WHISPER_TO], [UnicodeString(tmpStringA)]) +
                              ')[]: ' + tmpStringW;
              end;
          ...
          begin
            ...
            end else if tmpChatMode = cmWhisper then // Notify sender, when he is muted
...
begin
  ...
  Result := (aNetPlayerIndex <> -1) and
            (fMutedPlayersList.IndexOf(Pointer(fNetPlayers[aNetPlayerIndex].IndexOnServer)) <> -1);
end;
...
begin
  //Reconnection delay
  if (fReconnectRequested <> 0) and
     (GetTimeSince(fReconnectRequested) > RECONNECT_PAUSE) then DoReconnection;

  //Joining timeout
  if fNetGameState in [lgs_Connecting,lgs_Reconnecting,lgs_Query] then
    if (GetTimeSince(fJoinTimeout) > JOIN_TIMEOUT) and not fEnteringPassword and
       (fReconnectRequested = 0) then
      if Assigned(fOnJoinFail) then fOnJoinFail(gResTexts[TX_NET_QUERY_TIMED_OUT]);

  //Vote expiring
  if (fNetGameState in [lgs_Loading, lgs_Game]) and IsHost and
     fNetPlayers.VoteActive and (GetTimeSince(fLastVoteTime) > VOTE_TIMEOUT) then
  begin
