...
uses
  ...
  {$IFDEF WDC} ZLib {$ELSE} ZStream {$ENDIF}
  ;
...
function GetFullSourceFileName(aType: TKMTransferType; aName: UnicodeString; aMapFolder: TMapFolder;
                               Postfix, aExt: UnicodeString): UnicodeString;
begin
  case aType of
  ...
  case aType of
    kttMap:
      begin
        ...
        begin
          ...
          if FileExists(FileName) then AddFileToStream(FileName, '', VALID_MAP_EXTENSIONS[I]);
        end;
        ...
        begin
          ...
          begin
            repeat
              if (F.Attr and faDirectory = 0) then AddFileToStream(
                ExtractFilePath(FileName) + F.Name,
                ExtractFileExt(ChangeFileExt(F.Name,'')),
                VALID_MAP_EXTENSIONS_POSTFIX[I]
              );
            until FindNext(F) <> 0;
    ...
    kttSave:
      ...
      begin
        ...
        if FileExists(FileName) then AddFileToStream(FileName, '', VALID_SAVE_EXTENSIONS[I]);
      end;
  end;
...
begin
  ...
  if FileStream.Size > 0 then fSendStream.CopyFrom(FileStream, FileStream.Size);
  ...
  case fType of
    kttMap:
      begin
        ...
        if not DirectoryExists(FileName) then CreateDir(FileName);
        ...
        if not DirectoryExists(FileName) then
          ...
          begin
            repeat
              if (F.Attr and faDirectory = 0) then DeleteFile(FileName + PathDelim + F.Name);
            until FindNext(F) <> 0;
...
begin
  ...
  for I := Low(fSenders) to High(fSenders) do
    if fSenders[I] <> nil then Inc(Result);
end;
