...
uses
  Classes, SysUtils, Math, KM_CommonClasses, KM_NetworkClasses, KM_NetworkTypes, KM_Defaults, KM_Utils, VerySimpleXML,
     ...
     {$IFDEF WDC} KM_NetServerOverbyte {$ELSE} KM_NetServerLNet {$ENDIF}
     ;
...
type
  ...
  TKMNetServer = class
  private
    fServer:     {$IFDEF WDC} TKMNetServerOverbyte {$ELSE} TKMNetServerLNet {$ENDIF} ;
  ...
  public
    constructor Create(aMaxRooms, aKickTimeout: Word; aHTMLStatusFile, aWelcomeMessage: UnicodeString);
    destructor Destroy; override;
...
{ TKMNetServer }
constructor TKMNetServer.Create(aMaxRooms, aKickTimeout: Word; aHTMLStatusFile, aWelcomeMessage: UnicodeString);
begin
  inherited Create;
...
procedure TKMNetServer.UpdateSettings(aKickTimeout: Word; const aHTMLStatusFile: UnicodeString;
                                      const aWelcomeMessage: UnicodeString; const aServerName: AnsiString);
begin
  fKickTimeout    := aKickTimeout;
  fHTMLStatusFile := aHTMLStatusFile;
  fWelcomeMessage := aWelcomeMessage;
  ...
  case Kind of
    mk_JoinRoom:
      begin
        ...
        if InRange(tmpInteger, 0, Length(fRoomInfo)-1) and
           (fRoomInfo[tmpInteger].HostHandle <> NET_ADDRESS_EMPTY) and
           //Once game has started don't ask for passwords so clients can reconnect
           (fRoomInfo[tmpInteger].GameInfo.GameState = mgsLobby) and
           (fRoomInfo[tmpInteger].Password <> '') then
    ...
    mk_Password:
      begin
        ...
        if InRange(tmpInteger, 0, Length(fRoomInfo)-1) and
           (fRoomInfo[tmpInteger].HostHandle <> NET_ADDRESS_EMPTY) and
           (fRoomInfo[tmpInteger].Password = tmpStringA) then
          AddClientToRoom(aSenderHandle, tmpInteger)
        else
          SendMessage(aSenderHandle, mk_ReqPassword);
      end;
    ...
    mk_ResetBans: if SenderIsHost then SetLength(fRoomInfo[SenderRoom].BannedIPs, 0);
    mk_GetServerInfo:
      begin
        M2 := TKMemoryStream.Create;
        SaveToStream(M2);
...
begin
  ...
  Result := (aHandle = NET_ADDRESS_OTHERS) or (aHandle = NET_ADDRESS_ALL) or
            (aHandle = NET_ADDRESS_HOST) or (aHandle = NET_ADDRESS_SERVER) or
...
procedure TKMNetServer.SaveHTMLStatus;
  ...
  begin
    Result := '#' + IntToHex(aCol and $FF, 2) + IntToHex((aCol shr 8) and $FF, 2) +
              IntToHex((aCol shr 16) and $FF, 2);
  end;
